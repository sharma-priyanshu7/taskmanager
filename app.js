document.addEventListener('DOMContentLoaded', () => {
    ...
    function setActiveButton() {
        ...
        // date picker visibility and restrictions
        const today = todayStr();
        viewDateInput.value = ''; // Clear value to avoid stale data
        viewDateInput.removeAttribute('min'); // Clear min
        viewDateInput.removeAttribute('max'); // Clear max
        if (currentView === 'today') {
            viewDateInput.classList.add('hidden');
            viewDateInput.style.display = 'none'; // Extra assurance
            selectedDate = null;
            openAddBtn.style.display = 'inline-block'; // Add visible on today
        } else {
            viewDateInput.classList.remove('hidden');
            viewDateInput.style.display = 'inline-block'; // Ensure visibility
            openAddBtn.style.display = currentView === 'future' ? 'inline-block' : 'none';
            if (currentView === 'future') {
                viewDateInput.min = today; // No past dates
                viewDateInput.value = today; // Default to today
            } else if (currentView === 'past') {
                viewDateInput.max = today; // No future dates
                viewDateInput.value = today; // Default to today
            }
        }
    }
    ...
    function renderColumnsForDate(dateStr, opts = {}) {
        ...
        colsToShow.forEach(col => {
            ...
            // Only enable drag/drop in readOnly=false and only for main (today) view
    ...
    function openAddModal(prefill = {}) {
        ...
        taskDueInput.value = prefill.due || todayStr();
        // Restrict past dates in Today view for new tasks
        taskDueInput.removeAttribute('min'); // Clear any existing min
        if (currentView === 'today' && !prefill.due) {
            taskDueInput.min = todayStr(); // No past dates in Today
        }
    ...
    function openEditModal(id) {
        ...
        taskDueInput.removeAttribute('min'); // Allow past dates when editing
